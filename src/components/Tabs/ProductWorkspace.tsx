...
const ProductWorkspace: React.FC<Props> = ({ 
  ...
  // CRASH PROTECTION: Robust lookup
  ...
  // 2. DATA: Tab Groups (Using Translations with Safe Fallbacks)
      ...
      <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in h-full pb-10">
        ...
        <div className="lg:w-80 shrink-0 flex flex-col gap-6 sticky top-6 self-start h-fit">
           ...
           <div className="grid grid-cols-2 gap-4">
              {canSeeFinancials && (
                <div className="bg-[#003d5b] dark:bg-blue-600 p-5 rounded-[2rem] text-white shadow-lg flex flex-col justify-between min-h-[100px]">
              ...
              <div className={`bg-white dark:bg-slate-900 p-5 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex flex-col justify-between min-h-[100px] ${!canSeeFinancials ? 'col-span-2' : ''}`}>
                  ...
                  <p className="text-xs font-mono font-bold text-slate-800 dark:text-slate-200 truncate">{selectedProduct.hsCode || '---'}</p>
              </div>
           </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="bg-white dark:bg-slate-900 p-2 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-2">
              <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl shrink-0">
                  <button onClick={() => handleModeSwitch('ops')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${viewMode === 'ops' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
                      <ToolIcon className="w-4 h-4" /> {t?.opsMode || "Ops"}
                  </button>
                  <button onClick={() => handleModeSwitch('strat')} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${viewMode === 'strat' ? 'bg-gradient-to-r from-purple-500 to-indigo-500 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
                      <Sparkles className="w-4 h-4" /> {t?.strategyMode || "Strategy"}
                  </button>
              </div>
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 gap-4">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
          <CubeIcon className="w-7 h-7 text-[#003d5b] dark:text-blue-500" /> {t?.title || "Product Workspace"}
        </h2>
