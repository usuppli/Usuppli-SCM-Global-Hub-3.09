...
const GridIcon = ({ className }: { className: string }) => (<svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 01-2 h-2a2 2 0 01-2-2v-2z" /></svg>);
...
const ProductionFloor: React.FC<Props> = ({ jobs = [], lang = 'en' }) => {
  // --- TRANSLATION SETUP ---
  // This connects the component to your updated translation files
  const rootT = translations[lang] || translations['en'];
  const t = rootT?.shopFloor || translations['en'].shopFloor || {};
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-800">
         <div>
            ...
            <p className="text-slate-400 text-xs font-mono uppercase tracking-[0.2em]">
                {isConnected ? `${t.connected || 'Connected'}: ${activeMachine} • Node_ID: US-SCM-A4` : `${t.standby || 'Station Standby'} • Awaiting Signal`}
            </p>
         </div>
         
         <div className="flex gap-6 mt-6 md:mt-0">
             <div className="text-right hidden md:block border-r border-slate-800 pr-8">
                 <span className="block text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">{t.activeStream || "Active Stream"}</span>
             ...
             >
                 {isConnected ? (t.killConnection || 'Kill Connection') : (t.syncLine || 'Sync Line Status')}
             </button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl relative overflow-hidden">
              <div className="flex justify-between items-center mb-8">
                  <h4 className="text-white font-black text-sm uppercase tracking-[0.2em] flex items-center gap-3">
                      ...
                      {t.efficiencyMatrix || "Live Efficiency Matrix"}
                  </h4>
                  <div className="flex gap-3 text-[10px] text-slate-400 font-black font-mono">
                      <span className="bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">{t.oee || "OEE"}: 87.4%</span>
                      <span className="bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700 text-blue-400">{t.target || "TARGET"}: 92%</span>
                  </div>
              </div>
          ...
          <div className="space-y-6">
              <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl">
                  <h4 className="text-slate-500 text-[10px] font-black uppercase mb-6 tracking-[0.2em]">{t.envSensors || "Environmental Sensors"}</h4>
                  <div className="space-y-6">
              ...
              <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl">
                  <h4 className="text-slate-500 text-[10px] font-black uppercase mb-6 tracking-[0.2em]">{t.alerts || "Diagnostic Alerts"}</h4>
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm gap-4">
        <div>
           <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{t.queueStatus || "Production Queue Status"}</h3>
           <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest">{t.liveTracking || "Live factory throughput tracking"}</p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full xl:w-auto">
          <div className="relative flex-grow md:w-72 group">
            ...
            <input 
                ...
                placeholder={t.searchJobs || "Search Active Jobs..."}
                value={searchTerm} 
          ...
          {isAdmin && (
            ...
            >
                <DownloadIcon className="w-4 h-4" /> <span>{t.export || "Export CSV"}</span>
            </button>
          )}
        </div>
      </div>
