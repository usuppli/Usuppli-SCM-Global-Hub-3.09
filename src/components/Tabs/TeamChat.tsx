            ...
            <div className="flex-1 flex min-h-0">
                <div className="w-72 bg-slate-50 dark:bg-slate-950 border-r border-slate-100 dark:border-slate-800 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-6">
                        <div>
                            <h4 className="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">{t?.channels}</h4>
                        ...
                        <div>
                             <h4 className="px-3 mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">{t?.directMessages}</h4>
                             <div className="space-y-1">
                ...
                <div className="flex-1 flex flex-col bg-white dark:bg-slate-900 relative">
                    {activeThreadId ? (
                                ...
                                <div className="relative">
                                    ...
                                    {menuOpen && (
                                        <div className="absolute right-0 top-12 bg-white dark:bg-slate-800 shadow-2xl rounded-2xl border border-slate-100 dark:border-slate-700 p-2 w-48 z-50">
                                            <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 border-b border-slate-50 dark:border-slate-700 mb-1">{t?.shareVia}</p>
                                            <button onClick={() => handleShare('wechat')} className="w-full text-left px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-3 text-xs font-bold text-slate-700 dark:text-slate-200"><div className="text-emerald-500"><WeChatIcon className="w-4 h-4" /></div> {t?.wechat}</button>
                                            <button onClick={() => handleShare('whatsapp')} className="w-full text-left px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-3 text-xs font-bold text-slate-700 dark:text-slate-200"><div className="text-green-500"><WhatsAppIcon className="w-4 h-4" /></div> {t?.whatsapp}</button>
                                            <button onClick={() => handleShare('email')} className="w-full text-left px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-3 text-xs font-bold text-slate-700 dark:text-slate-200"><div className="text-blue-500"><EmailIcon className="w-4 h-4" /></div> {t?.email}</button>
                                            <button onClick={() => handleShare('sms')} className="w-full text-left px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-3 text-xs font-bold text-slate-700 dark:text-slate-200"><div className="text-slate-500"><SMSIcon className="w-4 h-4" /></div> {t?.sms}</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                                ...
                                <form onSubmit={handleSend} className="flex gap-2 bg-slate-50 dark:bg-slate-800 p-1.5 rounded-[1.2rem] border border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-[#003d5b]/10 dark:focus-within:ring-blue-500/20 transition-all items-center">
                                   ...
                                   {isRecording ? (
                                        <div className="flex-1 flex items-center gap-3 px-3 py-1 bg-red-50 dark:bg-red-900/20 rounded-xl text-red-600 dark:text-red-400 animate-pulse">
                                             <div className="w-2 h-2 bg-red-500 rounded-full animate-bounce"></div><span className="text-xs font-bold">{t?.recording}</span><div className="flex-1"></div><button type="button" onClick={() => setIsRecording(false)} className="text-[10px] font-bold text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">{t?.cancel}</button>
                                        </div>
                                   ) : (
                                        <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={t?.typeMessage} className="flex-1 bg-transparent border-none focus:ring-0 text-xs font-medium text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 outline-none" />
                                   )}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-300 dark:text-slate-700">
                             ...
                             <h3 className="text-lg font-black text-slate-800 dark:text-slate-200">{t?.title}</h3>
                             <p className="text-xs font-bold mt-1 text-slate-400 dark:text-slate-500">{t?.welcome}</p>
                        </div>
                    )}
                </div>
            </div>
...
const TeamChat: React.FC<TeamChatProps> = ({ currentUser, users, lang, isOpen, isMinimized, onMinimize, onClose, onUnreadChange, setGlobalMessages }) => {
  ...
  const [threads, setThreads] = useState<ChatThread[]>(() => {
    ...
    return [{ id: GROUP_ID, name: t.teamGeneral, type: 'group', unreadCount: 5, lastMessage: "Please check the new logistics report.", lastMessageTime: new Date(Date.now()-300000), isOnline: true }, ...dms];
  });
