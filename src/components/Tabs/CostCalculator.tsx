...
const DEFAULT_COMPONENTS = [
    "Materials", "Labor", "Packaging", "Custom Design", "Logistics", "Inspection", "Production", "Export Processing (China)", "Export Fee (International)"
];
...
interface CostLine { id: string; name: string; value: number; }

interface Props { product: Product; lang: Language; onSave: (product: Product) => void; isReadOnly?: boolean; }

const CostCalculator: React.FC<Props> = ({ product, lang, onSave, isReadOnly }) => {
  const rootT = translations[lang] || translations['en'];
  const t = rootT.workspace.costing;
  ...
  useEffect(() => {
    ...
    Object.entries(rawCosts).forEach(([key, value]) => {
        if (LOGISTICS_KEYS.includes(key)) { fixed[key] = value; } 
        else {
    ...
    DEFAULT_COMPONENTS.forEach(name => {
        ...
        if (!foundKeys.has(key)) { loadedLines.push({ id: key, name: name, value: 0 }); }
    });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className={`${getCardClassName(isEditingCosts)} lg:col-span-2`} onClick={() => !isEditingCosts && !isReadOnly && setIsEditingCosts(true)}>
              ...
              <h4 className="text-sm font-bold text-slate-800 dark:text-white mb-4 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 pb-2">{t.structure}</h4>
              <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                      <thead className="text-[10px] text-slate-400 uppercase font-bold bg-slate-50 dark:bg-slate-950/50">
                          <tr>
                              <th className="p-3 text-left rounded-l-lg">{t.table.component}</th>
                              <th className="p-3 text-right">{t.table.value}</th>
                              <th className="p-3 text-right rounded-r-lg w-24">{t.table.percent}</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                  ...
                                  <td className="p-3 pl-4 border-l-2 border-transparent group-hover:border-blue-500">
                                      {isEditingCosts ? (
                                          <div className="flex items-center gap-2">
                                              <button onClick={(e) => removeCostLine(index, e)} className="text-slate-400 hover:text-red-500 p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded transition-colors"><TrashIcon className="w-3.5 h-3.5" /></button>
                                              <input className={STYLES.inputName} value={line.name} placeholder="Component Name" onClick={(e) => e.stopPropagation()} onChange={(e) => updateCostLine(index, 'name', e.target.value)} />
                                          </div>
                                      ) : (<span className="font-medium text-slate-600 dark:text-slate-300 capitalize">{line.name}</span>)}
                                  </td>
                                  <td className="p-3 text-right">
                                      {isEditingCosts ? (<input type="number" className={STYLES.inputTable} value={line.value || ''} onClick={(e) => e.stopPropagation()} onChange={(e) => updateCostLine(index, 'value', parseFloat(e.target.value) || 0)} />) : (<span className="font-mono">${(line.value || 0).toFixed(2)}</span>)}
                                  </td>
                                  <td className="p-3 text-right text-xs text-slate-400">{totalLandedCost > 0 ? `${(((line.value || 0) / totalLandedCost) * 100).toFixed(1)}%` : '-'}</td>
                              </tr>
                          ))}
                          {isEditingCosts && (
                              <tr>
                                  <td colSpan={3} className="p-2 text-center">
                                      <button onClick={addCostLine} className="px-3 py-1.5 border border-dashed border-slate-300 dark:border-slate-700 text-slate-500 text-xs font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex items-center gap-2 mx-auto">
                                          <PlusIcon className="w-3.5 h-3.5" /> {t.addComponent}
                                      </button>
                                  </td>
                              </tr>
                          )}
                          <tr className="bg-slate-50/50 dark:bg-slate-800/20 font-bold">
                              <td className="p-3 text-slate-800 dark:text-white">{t.fob}</td>
                              <td className="p-3 text-right text-slate-800 dark:text-white font-mono">${subtotalFOB.toFixed(2)}</td>
                              <td className="p-3"></td>
                          </tr>
                          <tr>
                              <td className="p-3 font-medium text-slate-600 dark:text-slate-300 pl-4">{t.freight}</td>
                              <td className="p-3 text-right">
                                  {isEditingCosts ? <input type="number" className={STYLES.inputTable} value={logisticsCosts.freight || 0} onClick={(e) => e.stopPropagation()} onChange={e => setLogisticsCosts({...logisticsCosts, freight: parseFloat(e.target.value) || 0})} /> : <span className="font-mono">${(logisticsCosts.freight || 0).toFixed(2)}</span>}
                              </td>
                          ...
                          <tr>
                              ...
                              <td className="p-3 text-right"><span className="font-mono text-amber-600 dark:text-amber-400 font-bold">${calculatedDuty.toFixed(2)}</span></td>
                          ...
                          <tr className="bg-[#003d5b]/5 dark:bg-blue-900/20 border-t-2 border-[#003d5b] dark:border-blue-500">
                              <td className="p-4 text-base font-black text-[#003d5b] dark:text-blue-400 uppercase tracking-wide">{t.total}</td>
          ...
          <div className="flex flex-col gap-6">
              <div className={getCardClassName(isEditingLogistics)} onClick={() => !isEditingLogistics && !isReadOnly && setIsEditingLogistics(true)}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                          ...
                          {isEditingLogistics ? (<input type="number" className={STYLES.inputCard} value={dims.weightKg} onClick={(e) => e.stopPropagation()} onChange={e => setDims({...dims, weightKg: parseFloat(e.target.value)})} />) : (<div className="text-sm font-mono text-slate-700 dark:text-slate-300">{dims.weightKg} kg</div>)}
                      </div>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                      <div className="flex justify-between items-center mb-1"><span className="text-xs text-slate-500">{t.volumetric}:</span><span className="text-xs font-mono">{volumetricWeight.toFixed(2)} kg</span></div>
                      <div className="flex justify-between items-center border-t border-slate-200 dark:border-slate-700 pt-1 mt-1"><span className="text-xs font-bold text-slate-700 dark:text-slate-300">{t.chargeable}:</span><span className={`text-sm font-mono font-bold ${chargeableWeight > dims.weightKg ? 'text-red-500' : 'text-emerald-500'}`}>{chargeableWeight.toFixed(2)} kg</span></div>
                      {chargeableWeight > dims.weightKg && (<div className="flex items-center gap-1.5 mt-2 text-[10px] text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg"><AlertIcon className="w-3 h-3" /> {t.payingVolume}</div>)}
                  </div>
              </div>

              <div className="bg-[#003d5b] dark:bg-slate-800 rounded-2xl p-6 shadow-lg text-white relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-4 opacity-10"><TrendingIcon className="w-24 h-24" /></div>
                  <h4 className="text-sm font-bold uppercase tracking-wider mb-4 relative z-10">{t.profit}</h4>
                  <div className="space-y-4 relative z-10">
                      <div>
                          <label className="block text-[10px] font-bold text-sky-200 uppercase mb-1">{t.targetRetail}</label>
                          <div className="relative">
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-900 dark:text-white font-bold">$</span>
                              <input type="number" className="w-full p-2 pl-6 bg-white/10 border border-white/20 rounded-xl text-white font-bold outline-none focus:bg-white/20 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={targetRetail} onChange={e => setTargetRetail(parseFloat(e.target.value) || 0)} />
                          </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                          <div className="bg-white/10 rounded-xl p-3 backdrop-blur-sm"><p className="text-[10px] text-sky-200 uppercase">{t.margin}</p><p className={`text-xl font-black ${margin < 30 ? 'text-red-300' : 'text-emerald-300'}`}>{margin.toFixed(1)}%</p></div>
                          <div className="bg-white/10 rounded-xl p-3 backdrop-blur-sm"><p className="text-[10px] text-sky-200 uppercase">{t.netProfit}</p><p className="text-xl font-black">${profit.toFixed(2)}</p></div>
                      </div>
                  </div>
              </div>
          </div>
