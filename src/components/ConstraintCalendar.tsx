...
import { 
  ...
  Box, Hash, CheckCircle2, CalendarPlus
} from 'lucide-react';
...
const MOCK_EVENTS: ConstraintEvent[] = [
  {
    id: 'e1',
    title: 'LA Port Congestion',
    description: 'Moderate Delay Risk - Shipping',
    start: '2026-02-01',
    end: '2026-02-05',
    type: 'port_delay',
    severity: 'medium',
    region: 'North America',
    impactedArea: 'Logistics',
    linkedEntityType: 'Shipment',
    linkedEntityName: 'SH-9921',
    linkedEntityId: 'SH-9921'
  },
  {
    id: 'e2',
    title: 'CNY Factory Shutdown',
    description: 'All production halted',
    start: '2026-02-10',
    end: '2026-02-24',
    type: 'factory_shutdown',
    severity: 'high',
    region: 'Asia',
    impactedArea: 'Production'
  },
  {
    id: 'e3',
    title: 'Early Booking Discount',
    description: 'Maersk Spot Rates -15%',
    start: '2026-02-15',
    end: '2026-02-20',
    type: 'shipping_opportunity',
...
export default function ConstraintCalendar({ 
  ...
  const handleSaveCalculationToCalendar = () => {
    ...
    const calcEvent: ConstraintEvent = {
        ...
        description: `Planned shipment of ${calcQuantity} units to ${calcDestination}. Lead time: ${calcResults.totalDays} days.`,
        start: calcStartDate,
    ...
    setCurrentDate(new Date(calcStartDate));
  };
  ...
  const getEntityIcon = (type?: string) => {
    switch(type) {
      case 'Customer': return <User className="w-3 h-3" strokeWidth={1.5} />;
      case 'Job': return <Package className="w-3 h-3" strokeWidth={1.5} />;
      case 'Shipment': return <Truck className="w-3 h-3" strokeWidth={1.5} />;
      case 'Sample': return <TestTube className="w-3 h-3" strokeWidth={1.5} />;
      case 'Supplier': return <Factory className="w-3 h-3" strokeWidth={1.5} />;
      case 'Product': return <Box className="w-3 h-3" strokeWidth={1.5} />;
      default: return null;
    }
  };
    ...
    return (
      <div className="fixed bottom-6 right-24 z-[200] animate-in slide-in-from-bottom-10 fade-in duration-300">
        <button onClick={() => setIsMinimized(false)} className="bg-[#003d5b] hover:bg-[#002a40] text-white p-4 rounded-full shadow-2xl flex items-center gap-3 border border-white/20 transition-transform hover:scale-105 active:scale-95">
          <div className="relative"><Calendar className="w-6 h-6" strokeWidth={1.5} /><span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-[#003d5b]"></span></div>
          <span className="font-bold pr-2">Constraint Calendar</span><Maximize2 className="w-4 h-4 opacity-70" strokeWidth={1.5} />
        </button>
      </div>
    );
  }

  const containerClasses = isDocked ? "fixed right-0 top-0 h-full w-[500px] z-[100] shadow-2xl animate-in slide-in-from-right duration-300" : "fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6 animate-in fade-in duration-200";
      ...
      <div className={windowClasses}>
        ...
        <div className="bg-[#003d5b] p-3 flex justify-between items-center text-white shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/10 rounded-lg"><Calendar className="w-5 h-5" strokeWidth={1.5} /></div>
            <div>
              <h3 className="text-lg font-bold leading-none">Constraint Calendar</h3>
              <p className="text-white/60 text-xs mt-0.5">Global Production & Logistics Constraints</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
             <button onClick={() => setShowCalculator(!showCalculator)} className={`p-2 hover:bg-white/10 rounded-lg transition-colors text-white/80 hover:text-white mr-1 ${showCalculator ? 'bg-white/20 text-white' : ''}`} title="Lead Time Calculator & Scheduler">
                <Calculator className="w-5 h-5" strokeWidth={1.5} />
             </button>
             <button onClick={() => setShowAddModal(true)} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-xs font-bold transition-colors flex items-center gap-2 border border-white/10 mr-2"><Plus className="w-3 h-3" strokeWidth={1.5} /> Add Event</button>
             <button onClick={() => setIsDocked(!isDocked)} className={`p-2 hover:bg-white/10 rounded-lg transition-colors ${isDocked ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white'}`} title={isDocked ? "Center Window" : "Dock to Right"}><PanelRight className="w-5 h-5" strokeWidth={1.5} /></button>
             <button onClick={() => setIsMinimized(true)} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-white/80 hover:text-white" title="Minimize"><Minus className="w-5 h-5" strokeWidth={1.5} /></button>
             <button onClick={onClose} className="p-2 hover:bg-red-500/80 rounded-lg transition-colors text-white/80 hover:text-white" title="Close"><X className="w-5 h-5" strokeWidth={1.5} /></button>
          </div>
        </div>
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden">
            <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shrink-0">
                ...
                <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                    <button onClick={handlePrevMonth} className="p-1 hover:bg-white dark:hover:bg-slate-700 rounded-md shadow-sm transition-all"><ChevronLeft className="w-5 h-5 text-slate-600 dark:text-slate-300" strokeWidth={1.5} /></button>
                    ...
                    <button onClick={handleNextMonth} className="p-1 hover:bg-white dark:hover:bg-slate-700 rounded-md shadow-sm transition-all"><ChevronRight className="w-5 h-5 text-slate-600 dark:text-slate-300" strokeWidth={1.5} /></button>
                </div>
                {!isDocked && (
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-red-50 border border-red-100"><div className="w-2 h-2 rounded-full bg-red-500"></div> High</div>
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-amber-50 border border-amber-100"><div className="w-2 h-2 rounded-full bg-amber-500"></div> Medium</div>
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-emerald-50 border border-emerald-100"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> Opp.</div>
                        <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-indigo-50 border border-indigo-100"><div className="w-2 h-2 rounded-full bg-indigo-500"></div> Plan</div>
                    </div>
                )}
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4">
                <div className="grid grid-cols-7 gap-px bg-slate-200 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="bg-slate-50 dark:bg-slate-800 p-2 text-center text-xs font-bold uppercase text-slate-400 dark:text-slate-500">{isDocked ? day.charAt(0) : day}</div>
                ))}
                
                {days.map((day, index) => {
                    if (!day) return <div key={`empty-${index}`} className="bg-white dark:bg-slate-900 min-h-[80px] sm:min-h-[100px]" />;
                    const dayEvents = getEventsForDay(day);
                    const isToday = day === new Date().getDate() && currentDate.getMonth() === new Date().getMonth();
                    return (
                    <div key={day} className={`bg-white dark:bg-slate-900 min-h-[80px] sm:min-h-[100px] p-2 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 relative group ${isToday ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''}`}>
                        <span className={`text-sm font-bold ${isToday ? 'text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 w-7 h-7 flex items-center justify-center rounded-full' : 'text-slate-700 dark:text-slate-300'}`}>{day}</span>
                        <div className="mt-2 space-y-1">
                        {dayEvents.map((event) => (
                            <div key={event.id} onMouseEnter={() => setHoveredEvent(event)} onMouseLeave={() => setHoveredEvent(null)} className={`text-[10px] px-1.5 py-1 rounded border-l-2 truncate cursor-help flex items-center gap-1 shadow-sm transition-all hover:-translate-y-0.5 ${SEVERITY_STYLES[event.severity].bg} ${SEVERITY_STYLES[event.severity].text} ${SEVERITY_STYLES[event.severity].border} border-l-${SEVERITY_STYLES[event.severity].dot.replace('bg-', '')}`}>
                            {event.linkedEntityType && <span className="opacity-70 shrink-0">{getEntityIcon(event.linkedEntityType)}</span>}
            ...
            <div className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 text-[10px] text-slate-400 flex justify-between items-center shrink-0">
             <span>Data updated: 10 mins ago via API</span>
             <span className="font-mono">Global Context</span>
            </div>
          </div>

          {/* RIGHT SIDE: CALCULATOR (REVISED HEADER) */}
          {showCalculator && (
             <div className="w-96 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col animate-in slide-in-from-right duration-300 shadow-xl z-20">
                {/* Header: Revised to remove gray background */}
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center shrink-0">
                    <h3 className="font-bold text-sm flex items-center gap-2 text-slate-800 dark:text-slate-100">
                      <Calculator className="w-4 h-4 text-blue-600" strokeWidth={1.5} /> 
                      Lead Time Calculator
                    </h3>
                    <button onClick={() => setShowCalculator(false)} className="text-slate-400 hover:text-red-500 transition-colors">
                ...
                <div className="p-4 space-y-4 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div className="space-y-3 pb-4 border-b border-slate-50 dark:border-slate-800">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Calculation For</label>
                            <div className="flex gap-2">
                                {['Job', 'Sample', 'Product'].map((t) => (
                                    <button 
                                        key={t}
                                        onClick={() => setCalcType(t as any)}
                                        className={`flex-1 py-1.5 text-xs font-bold rounded-lg border transition-all ${calcType === t ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                                    >
                                        {t}
                                    </button>
                                ))}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                >
                                    <option value="">Select {calcType}...</option>
                                ...
                                <div className="absolute left-2.5 top-2.5 text-slate-400">
                                    {calcType === 'Job' ? <Package className="w-4 h-4" strokeWidth={1.5} /> : calcType === 'Sample' ? <TestTube className="w-4 h-4" strokeWidth={1.5} /> : <Box className="w-4 h-4" strokeWidth={1.5} />}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Breakdown (Days)</label>
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2"><Factory className="w-3 h-3" strokeWidth={1.5} /> Production</span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2"><CheckCircle2 className="w-3 h-3" strokeWidth={1.5} /> Quality Check</span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2"><Ship className="w-3 h-3" strokeWidth={1.5} /> Shipping</span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-slate-600 dark:text-slate-400 flex items-center gap-2"><FileText className="w-3 h-3" strokeWidth={1.5} /> Customs</span>
                    ...
                    <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800 mt-2 shrink-0">
                        <div className="flex justify-between items-end mb-2">
                            <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">Total Days</span>
                        ...
                        <div className="pt-2 border-t border-blue-200 dark:border-blue-800 flex justify-between items-center">
                            <span className="text-xs font-bold text-slate-500">Est. Arrival (ETA)</span>
                            <span className="text-sm font-bold text-blue-700 dark:text-blue-300">{calcResults.eta.toDateString()}</span>
                        </div>
                        <button onClick={handleSaveCalculationToCalendar} className="w-full mt-3 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all shadow-md active:scale-95">
                            <CalendarPlus className="w-4 h-4" strokeWidth={2} /> Add Plan to Calendar
                        </button>
                    </div>
                </div>
             </div>
            ...
            <div className="absolute inset-0 z-[160] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
                <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4">
                    <div className="flex justify-between items-center border-b border-slate-50 dark:border-slate-800 pb-3">
                        <h3 className="font-bold text-lg dark:text-white">Add Constraint / Event</h3>
                        <button onClick={() => setShowAddModal(false)}><X className="w-5 h-5 text-slate-400" strokeWidth={1.5} /></button>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-50 dark:border-slate-800">
                        <div className="flex items-center gap-2 mb-2 text-blue-600"><Link className="w-4 h-4" strokeWidth={1.5} /><span className="text-xs font-bold uppercase">Link to Record (Optional)</span></div>
                        <div className="grid grid-cols-2 gap-4">
                    ...
                    <button onClick={handleAddEvent} className="w-full py-2.5 bg-[#003d5b] text-white rounded-lg font-bold hover:bg-[#002a40] flex items-center justify-center gap-2 transition-all shadow-md active:scale-95"><Save className="w-4 h-4" strokeWidth={1.5} /> Save Event</button>
                </div>
            </div>
        )}
        ...
        {hoveredEvent && !showAddModal && (
          <div className="fixed z-[150] bg-white dark:bg-slate-800 p-4 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 w-64 pointer-events-none animate-in fade-in zoom-in-95" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
            <div className="flex items-center gap-2 mb-2">
              <div className={`p-1.5 rounded-lg border ${SEVERITY_STYLES[hoveredEvent.severity].bg} ${SEVERITY_STYLES[hoveredEvent.severity].border} ${SEVERITY_STYLES[hoveredEvent.severity].text}`}><Info className="w-4 h-4" strokeWidth={1.5} /></div>
